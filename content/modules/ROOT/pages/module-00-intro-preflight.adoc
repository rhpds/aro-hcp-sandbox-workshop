...
[source,bash,role=execute]
----
az identity list --resource-group {azure_resource_group} --output table
----
...
az identity show --resource-group $RG \
  --name $(az identity list --resource-group {azure_resource_group} --query "[0].name" --output tsv) \
  --output yaml
----
...
[source,bash,role=execute]
----
IDENTITY_NAME=$(az identity list --resource-group {azure_resource_group} --query "[0].name" --output tsv)
