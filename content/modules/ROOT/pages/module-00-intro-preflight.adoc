...
Locate the Managed Identity resource in your resource group using the same `az resource list` command you used earlier, filtered by resource type:
...
Retrieve its full configuration by name:

[source,bash,role=execute]
----
IDENTITY_NAME=$(az resource list --resource-group {azure_resource_group} \
  --resource-type Microsoft.ManagedIdentity/userAssignedIdentities \
  --query "[0].name" --output tsv)
az identity show --resource-group {azure_resource_group} \
  --name $IDENTITY_NAME \
  --output yaml
----
